...
{
	unsigned int tyear[2] = {1, 9}, tmon = 12, tday[2] = { 1, 8}, tweek = 3;
	unsigned int thour[2] = {1, 0}, tmin[2] = { 2, 3 }, tsec[2] = {2, 1};
...
void DisplayTime(void) {
	...
	// 設置第一行 Cursor 起始位置
	SetCursor(0, 0x1); //WriteIns(0x8000);
	...
	WriteData(0x32); //字元"2"
	WriteData(0x30); //字元"0"
	// 因為是BCD編碼，所以要取十位數時要先往右位移4位，再轉成ASCII
	...
	// 設置第二行 Cursor 起始位置
	SetCursor(1, 0x6); //WriteIns(0xC000);
