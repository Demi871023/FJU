...
int main()
{
	...
	for (i=0; i<3; i++)
	{	
		GPIO_PTB_GPIO = 0xFFFF;		//燈全滅
		delay1(400000);
		GPIO_PTB_GPIO = 0x0000;		//燈全亮
		delay1(400000);
	}

	//邊吃邊長大
	unsigned int body = 0xC000;		//初始生命值
	unsigned int food = 0x1000;		//食物初始位置
	for ( i=0; i<7; i++ )
	{
		GPIO_PTB_GPIO = ~(body | food);		//亮出食物與蛇生命的燈	
		delay1(1000000);
	...
	for ( i = 9; i > 0; i-- )
	{
		...
		for ( j = 3; j > 0; j--)
		{
			GPIO_PTB_GPIO = ~(body);	//生命值亮
			delay1(50000);
			GPIO_PTB_GPIO = 0xFFFF;		//生命值暗
			delay1(50000);
		}
		...
		for ( j = i; j > 0; j-- )
		{
			GPIO_PTB_GPIO = ~(body ^ temp);		//做出生命值減一的效果
			delay1(1000000);
			temp = temp << 1;	
		}
		body = body >> 1;		
