...
int main()
{
	...
	unsigned int number[2][8] = {Number_4,Number_0,Number_6,Number_2,Number_6,Number_2,Number_2,Number_1,
			Number_4,Number_0,Number_6,Number_2,Number_6,Number_2,Number_1,Number_9
	};
	...
	while(1)
	{
		tmp = (GPIO_PTC_PADIN >> 2) & 0x3; //0x3表示dip switch只有PC2和PC3有作用
		...
		if( tmp == 0x1)
		{
			for(j = 1 ; j <= 8 ; j++) // j 決定一round亮幾個LED燈
			{
				for(myi = 0 ; myi <1000 ; myi++) // 在j格範圍內重複閃爍myi次，使得視覺上能夠有數字在板上停留的效果
				{
					for(i = j, k = 0 ; i > 0 ; i--, k++) // i決定哪一格燈會亮，k決定亮哪一個數字
					{
						GPIO_PTD_GPIO = number[0][k];
						GPIO_PTA_GPIO = index_7LED[i-1];
						delay1(100);
					}
				}
			}
			while(((GPIO_PTC_PADIN >> 2) & 0x3) == 0x1)
			{
				for(i = 0 ; i < 8 ; i++)
				{
					GPIO_PTD_GPIO = number[0][i];
		...
		if( tmp == 0x2)
		{
			for(j = 1 ; j <= 8 ; j++) // j 決定一round亮幾個LED燈
			{
				...
				{
					for(i = 8, k = 8-j ; i > (8-j) ; i--, k++ )
					{
						GPIO_PTD_GPIO = number[1][k];
						GPIO_PTA_GPIO = index_7LED[i-1];
						delay1(100);
					}
				}
			}
			while(((GPIO_PTC_PADIN >> 2) & 0x3) == 0x2)
			{
				for(i = 0 ; i < 8 ; i++)
				{
					GPIO_PTD_GPIO = number[1][i];
					GPIO_PTA_GPIO = index_7LED[7-i];
					delay1(100);
				}
			}
