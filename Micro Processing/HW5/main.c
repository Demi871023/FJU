...
unsigned int Close_Cursor_Flag = 0;
unsigned int Return_Home_Flag = 0;

void SetCursor(unsigned short Line, unsigned short Position)
{
	unsigned short Address = Line * 0x0040 + Position * 0x0001;
	WriteIns(0x80 | Address);
}

void Display_First_Line()
{
  //char ABCD[]={'A','N','D','E','S'};
  char Line1[]="AbCdEfGhIjKlMnOp";
  char Line2[]="aBcDeFgHiJKimNo";
  char i;
  WriteIns(0x38);  //FUNCTION SET
  WriteIns(0x0E);  //DISPLAY CONTROL
  WriteIns(0x06);  //SET INPUT MODE

  SetCursor(0,0);  //1-LINE DD RAM SET Address
  for(i=0;i<16;i++)
  {
	  WriteData(Line1[i]);
	  delay1(200);
...
void Display_Second_Line()
{
	...
	char Line1[]="123456789101112!";
	  char Line2[]="211101987654321";
	  char i;
	  WriteIns(0x38);  //FUNCTION SET
	  WriteIns(0x0E);  //DISPLAY CONTROL
	  WriteIns(0x06);  //SET INPUT MODE
...
void Close_Cursor()
{
	if(!Close_Cursor_Flag)	//關閉游標
		WriteIns(0x0C);
	else					//顯示游標
		WriteIns(0x0E);
	Close_Cursor_Flag = ~(Close_Cursor_Flag);
}
...
int main()
{
	...
	unsigned int First_Display_Flag = 0;
	unsigned int Second_Display_Flag = 0;

	InitialLCD();

	unsigned int tmp = 0;
	unsigned int Button_One_Click = 0;
	...
	unsigned int clickShiftButton = 0;
	...
	while(1)
	{
		...
		if (key != 0xFF)
		{
			if(!check)
			{
				...
				{	
					//顯示第一組文字
					First_Display_Flag = 1;
					Second_Display_Flag = 0;
					Display_First_Line();
					clickShiftButton = 0;
				}
				if(key == 1)
				{	
					//顯示第二組文字
					First_Display_Flag = 0;
					Second_Display_Flag = 1;
					Display_Second_Line();
					clickShiftButton = 0;
				}
				if(key == 2)	
		...
		{	
			//當按了shift鍵(2或3) 但還沒按下一個鍵
			//仍持續做shift的動作
			if(clickShiftButton == 1)
			{
				//判斷持續做左旋還是右旋
				if(LeftShift == 1 && RightShift == 0)
				{
					WriteIns(0x18);
					delay1(300000);
					clickShiftButton = 1;
				}
				if(LeftShift == 0 && RightShift == 1)
				{
					WriteIns(0x1C);
					delay1(300000);
					clickShiftButton = 1;
