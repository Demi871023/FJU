...
# 計算 F(x) 微分後在 X 點的數值
def FxDerAnswer(x):
    return(math.exp(x) + math.cos(2 * x) + 6 * x * math.sin(2 * x))
# 計算 F(x) 微分後在 X 點的數值
def FxAnswer(x):
...
def GxAnswer(x):
    ...
    #return((math.exp(x) - 8.3) / (3 * math.cos(2 * x)))
...
def SetUpPicXY():
    ...
    plt.title('f(x) = e^x - 3x * cos(2x) - 8.3 in f(-10) ~ f(2) value')
    plt.xlabel('x')
    plt.ylabel('f(x)')
    plt.savefig('F(x)ValuePic_One.png')
