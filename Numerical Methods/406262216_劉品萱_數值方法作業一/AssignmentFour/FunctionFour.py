...
# 計算 F(x) 微分後在 X 點的數值
def FxDerAnswer(x):
    return(math.exp(x) + 10 * x * math.sin(4 * x) + 20 * x * x * math.cos(4 * x))
# 計算 F(x) 微分後在 X 點的數值
def FxAnswer(x):
    return(math.exp(x) + 5 * x * x * math.sin(4 * x) - 2)

def GxAnswer(x):
    return(math.log(-5 * x * x * math.sin(4 * x) + 2))
...
def SetUpPicXY():
    ...
    plt.title('f(x) = e^x + 5 * x^2 * sin(4x) - 2 in f(-2) ~ f(1) value')
    plt.xlabel('x')
    plt.ylabel('f(x)')
    plt.savefig('F(x)ValuePic_Four.png')
