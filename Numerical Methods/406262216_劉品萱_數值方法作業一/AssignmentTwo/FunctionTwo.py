...
# 計算 F(x) 微分後在 X 點的數值
def FxDerAnswer(x):
    return((math.exp(x * math.sin(x)) * math.sin(x)) + (math.exp(x * math.sin(x)) * x * math.cos(x)) - (math.cos(2 * x)) + (2 * x * math.sin(2 * x)))
# 計算 F(x) 微分後在 X 點的數值
def FxAnswer(x):
...
def GxAnswer(x):
    ...
    #return((math.exp(x * math.sin(x)) - 2.8) / math.cos(2 * x)) 
...
def SetUpPicXY():
    ...
    plt.title('f(x) = e^(x * sinx) - x * cos(2x) - 2.8 in f(-5) ~ f(5) value')
    plt.xlabel('x')
    plt.ylabel('f(x)')
    plt.savefig('F(x)ValuePic_Two.png')
