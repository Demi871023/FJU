...
void ErrorTest()
{
    ...
    for(int i = 0 ; i < OutputLayerNeuron ; i++)
    {
        double err = sqrt(ErrorTotal[i][0] / 120);
        if(err >= tao)
            ErrorCount++;
    }
    if(ErrorCount == 0)
...
void ErrorCount(int n)
{
    ...
    {
        if(i == 0)
            ErrorTotal[i][0] = ErrorTotal[i][0] + pow((TrainingP[n].t1 - a[i][0]),2);
        if(i == 1)
            ErrorTotal[i][0] = ErrorTotal[i][0] + pow((TrainingP[n].t2 - a[i][0]),2);
        if(i == 2)
            ErrorTotal[i][0] = ErrorTotal[i][0] + pow((TrainingP[n].t3 - a[i][0]),2);
    }
